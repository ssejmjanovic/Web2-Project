...
namespace TravelService.Controllers
{
    ...
    public class DestinationsController : ApiControllerBase
    {
        ...
        public async Task<ActionResult<List<DestinationDto>>> GetAll(int planId)
            => Ok(await _destinationService.GetForPlanAsync(planId, CurrentUserId));

        [HttpGet("{id}")]
        public async Task<ActionResult<DestinationDto>> GetById(int planId, int id)
            => Ok(await _destinationService.GetByIdAsync(planId, id, CurrentUserId));

        [HttpPost]
        public async Task<ActionResult<DestinationDto>> Create(int planId, [FromBody] DestinationInputDto dto)
        {
            var created = await _destinationService.CreateAsync(planId, CurrentUserId, dto);
        ...
        public async Task<ActionResult<DestinationDto>> Update(
            ...
            => Ok(await _destinationService.UpdateAsync(planId, id, CurrentUserId, dto));

        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(int planId, int id)
        {
            await _destinationService.DeleteAsync(planId, id, CurrentUserId);
            return NoContent();
        }
    }
}
