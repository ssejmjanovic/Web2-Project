...
namespace TravelService.Controllers
{
    ...
    public class ActivitiesController : ApiControllerBase
    {
        ...
        public async Task<ActionResult<List<ActivityDto>>> GetAll(int planId)
            => Ok(await _activityService.GetForPlanAsync(planId, CurrentUserId));

        [HttpGet("{id}")]
        public async Task<ActionResult<ActivityDto>> GetById(int planId, int id)
            => Ok(await _activityService.GetByIdAsync(planId, id, CurrentUserId));

        [HttpPost]
        public async Task<ActionResult<ActivityDto>> Create(int planId, [FromBody] ActivityInputDto dto)
        {
            var created = await _activityService.CreateAsync(planId, CurrentUserId, dto);
        ...
        public async Task<ActionResult<ActivityDto>> Update(
            ...
            => Ok(await _activityService.UpdateAsync(planId, id, CurrentUserId, dto));

        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(int planId, int id)
        {
            await _activityService.DeleteAsync(planId, id, CurrentUserId);
            return NoContent();
        }
    }
}
