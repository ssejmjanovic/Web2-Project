...
namespace TravelService.Controllers
{
    ...
    public class ChecklistController : ApiControllerBase
    {
        ...
        public async Task<ActionResult<List<ChecklistItemDto>>> GetAll(int planId)
            => Ok(await _checklistService.GetForPlanAsync(planId, CurrentUserId));

        [HttpGet("{id}")]
        public async Task<ActionResult<ChecklistItemDto>> GetById(int planId, int id)
            => Ok(await _checklistService.GetByIdAsync(planId, id, CurrentUserId));

        [HttpPost]
        public async Task<ActionResult<ChecklistItemDto>> Create(int planId, [FromBody] ChecklistItemInputDto dto)
        {
            var created = await _checklistService.CreateAsync(planId, CurrentUserId, dto);
        ...
        public async Task<ActionResult<ChecklistItemDto>> Update(
            ...
            => Ok(await _checklistService.UpdateAsync(planId, id, CurrentUserId, dto));

        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(int planId, int id)
        {
            await _checklistService.DeleteAsync(planId, id, CurrentUserId);
            return NoContent();
        }
    }
}
